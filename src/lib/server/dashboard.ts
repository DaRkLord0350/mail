...
export async function getDashboard(): Promise<DashboardDTO> {
  ...
  const [usage, leads, deliveryGroups, campaigns, recent, worker, templates, campaignCount] = await Promise.all([
    ...
    prisma.campaign.findMany({
      ...
      take: 10,
      where: { status: { in: ["RUNNING", "SCHEDULED", "PAUSED", "READY", "DRAFT", "COMPLETED", "STOPPED"] } },
    }),
  ...
  for (const g of deliveryGroups) {
    counts[g.status as DeliveryStatus] = g._count._all;
    total += g._count._all;
  }
  ...
  return {
    ...
    worker: {
      ...
      nextSendAt: iso(worker?.nextSendAt),
      lastResult: (worker?.lastResult as unknown as WorkerResultDTO | null) ?? null,
      haltedReason: worker?.haltedReason ?? null,
    },
    setup: {
