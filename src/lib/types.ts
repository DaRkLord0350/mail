...
export interface LeadDTO {
  id: string; email: string; firstName: string | null; lastName: string | null; companyName: string | null; displayName: string | null;
  website: string | null; customDomain: string | null; phone: string | null; status: LeadStatus; lastSentAt: string | null;
  sendCount: number; metadata: Record<string, string>; createdAt: string; suppressed: boolean; lastCampaign: { id: string; name: string } | null;
}

export interface ImportIssue {
  row: number;
  type: "invalid_email" | "missing_email" | "duplicate_in_file" | "blank_row" | "malformed_row" | "missing_name" | "missing_company" | "already_contacted" | "suppressed";
  message: string;
  email?: string;
}
...
export interface ImportSummaryDTO {
  id: string; fileName: string; totalRows: number; validLeads: number; createdLeads: number; updatedLeads: number; invalidEmails: number;
  duplicateEmails: number; blankRows: number; malformedRows: number; missingNames: number; missingCompanies: number; alreadyContacted: number;
  readyToSend: number; issues: ImportIssue[]; createdAt: string;
}
...
export interface CampaignDTO {
  id: string; name: string; status: CampaignStatus; templateId: string | null; subjectTemplate: string; bodyTemplate: string; dailyLimit: number;
  scheduledAt: string | null; timezone: string; lastError: string | null; createdAt: string; updatedAt: string; startedAt: string | null;
  pausedAt: string | null; completedAt: string | null; recipientCount: number; counts: Record<DeliveryStatus, number>; sendingHaltedReason: string | null;
}
...
export interface CampaignPreviewDTO {
  ...
  summary: {
    recipients: number; willSend: number; willSkip: number; suppressed: number; alreadySent: number; missingVariables: number;
    queuedElsewhere: number; unknownVariables: string[]; effectiveDailyLimit: number; remainingToday: number; estimatedDays: number;
  };
}

export interface DeliveryDTO {
  id: string; campaignId: string; campaignName: string; leadId: string; email: string; leadName: string | null; companyName: string | null;
  renderedSubject: string; status: DeliveryStatus; providerMessageId: string | null; attempts: number; errorMessage: string | null;
  errorCode: string | null; sentAt: string | null; lastAttemptAt: string | null; nextAttemptAt: string; createdAt: string;
}
...
export interface WorkerResultDTO {
  startedAt: string; finishedAt: string; recovered: number; promotedCampaigns: number; completedCampaigns: number; processed: number;
  sent: number; failed: number; skipped: number; retried: number;
  stopReason: "batch_complete" | "queue_empty" | "quota_reached" | "time_budget" | "paced" | "auth_error" | "not_configured" | "error";
  message: string; usage: UsageDTO;
}
...
export interface DashboardDTO {
  ...
  recent: DeliveryDTO[];
  worker: { lastRunAt: string | null; nextSendAt: string | null; lastResult: WorkerResultDTO | null; haltedReason: string | null; };
  setup: { resendConfigured: boolean; senderConfigured: boolean; hasLeads: boolean; hasTemplates: boolean; hasCampaigns: boolean; };
}
