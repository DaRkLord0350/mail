...
function decodePdf(dataBase64: string): Buffer {
  ...
  if (data.length === 0 || data.length > MAX_PDF_BYTES) throw new Error("PDF must be smaller than 8 MB.");
  if (data.subarray(0, 4).toString("ascii") !== "%PDF") throw new Error("The selected file is not a valid PDF.");
  return data;
}
...
export const PUT = handler<{ id: string }>(async (req, { params }) => {
  ...
  try { decodePdf(input.dataBase64); } catch (e) { throw new Error(e instanceof Error ? e.message : String(e)); }
