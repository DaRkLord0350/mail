...
export function TemplateAttachment({ templateId }: { templateId: string }) {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await api.get<{ attachment: AttachmentDTO | null }>(`/api/templates/${templateId}/attachment`);
      setAttachment(r.attachment);
    } catch (e) {
      toast.error(`Couldn't load attachment: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function upload(file: File) {
    ...
    if (file.size > MAX_BYTES) {
      toast.error("PDF must be smaller than 8 MB.");
      return;
    }
    setBusy(true);
    try {
  ...
  async function remove() {
    setBusy(true);
    try {
      await api.del(`/api/templates/${templateId}/attachment`);
      setAttachment(null);
        ...
        {loading ? <p className="text-sm text-slate-500">Loading attachment…</p> : attachment ? (
          <div className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              <div className="rounded-lg bg-white p-2 shadow-sm"><FileText className="size-5 text-red-600" /></div>
              <div className="min-w-0"><p className="truncate text-sm font-medium text-slate-900">{attachment.name}</p><p className="text-xs text-slate-500">PDF · {formatBytes(attachment.sizeBytes)}</p></div>
            </div>
